# Makefile for feedback-one development
# 適用於 feedback-one 專案開發
...
test-web: ## Run Web UI tests (continuous)
	uvx --no-cache --with-editable . feedback-one test --web

test-desktop-func: ## Run desktop application functional tests
	uvx --no-cache --with-editable . feedback-one test --desktop
