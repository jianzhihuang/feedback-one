...
注意在任務完成前根據feedback-one工具的要求進行工具調用
```
...
```bash
# 版本查詢
uvx feedback-one@latest version       # 檢查版本

# 介面測試
uvx feedback-one@latest test --web    # 測試 Web UI (自動持續運行)
uvx feedback-one@latest test --desktop # 測試桌面應用程式 (v2.5.0 新功能)

# 調試模式
MCP_DEBUG=true uvx feedback-one@latest test

# 指定語言測試
MCP_LANGUAGE=en uvx feedback-one@latest test --web    # 強制使用英文介面
MCP_LANGUAGE=zh-TW uvx feedback-one@latest test --web  # 強制使用繁體中文
MCP_LANGUAGE=zh-CN uvx feedback-one@latest test --web  # 強制使用簡體中文
```
...
uvx --no-cache --with-editable . feedback-one test --web   # Web UI 測試 (持續運行)
uvx --no-cache --with-editable . feedback-one test --desktop # 桌面應用測試
...
A: v2.4.0 版本已完全移除 PyQt6 GUI 依賴。如需使用舊版 GUI，請指定 v2.3.0 或更早版本：`uvx feedback-one@2.3.0`
...
**Q: 中文字符亂碼**
A: 已在 v2.0.3 修復。更新到最新版本：`uvx feedback-one@latest`
...
- 🎯 自動聚焦輸入框功能 ([PR #39](https://github.com/Minidoracat/feedback-one/pull/39))

**leo108** - [GitHub @leo108](https://github.com/leo108)
- 🌐 SSH 遠端開發支援 (`MCP_WEB_HOST` 環境變數) ([PR #113](https://github.com/Minidoracat/feedback-one/pull/113))

**Alsan** - [GitHub @Alsan](https://github.com/Alsan)
- 🍎 macOS PyO3 編譯配置支援 ([PR #93](https://github.com/Minidoracat/feedback-one/pull/93))

**fireinice** - [GitHub @fireinice](https://github.com/fireinice)
- 📝 工具文檔優化 (LLM 指令移至 docstring) ([PR #105](https://github.com/Minidoracat/feedback-one/pull/105))
